...
const float MAX_NOISE_LIMIT = 80.0; 
    ...
    if (deviceConnected) {
      if ((millis() - connectionStartTime) > WARMUP_TIME) {
         if (sendCounter >= SEND_INTERVAL) {
            sendCounter = 0;
            String allData1 = String(a.acceleration.x, 2) + "," + String(a.acceleration.y, 2) + "," + String(a.acceleration.z, 2) + "," +
                              String(objTemp, 2) + "," + String(ambTemp, 2) + "," + String(fallStatus);
            String allData2 = String(objTemp, 2) + "," + String(ambTemp, 2) + "," + String(fallStatus) + "\n";
            pTxCharacteristic->setValue(allData2.c_str());
            pTxCharacteristic->notify();
            //Serial.println(allData1);
         }
      }
    }
  }
